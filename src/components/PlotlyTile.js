...
class PlotlyTileComponent extends Component {
  componentWillMount() {
    ...
    // );
    // (this.props.tile.data || []).forEach( dataItem =>{
    //   console.log('dataItem tile', dataItem);
    //   dataItem.xy && this.props.getTimeseriesMetadataAction(dataItem.xy.uuid)
    // });
    this.timeseries().forEach(dataItem => {
      console.log("dataItem tile", dataItem);
      this.props.getTimeseriesMetadataAction(dataItem);
    });

    // this.props.getTimeseriesMetadataAction(this.props.tile.data.xy.uuid)
  ...
  // }
  timeseries() {
    // use data array from tile
    return (
      (this.props.tile.data || [])
        // filter out items that do not have a xy field (this field contains the uuid of the timeseries)
        .filter(e => e.xy)
        // convert to uuid
        .map(e => e.xy.uuid)
    );
  ...
  render() {
    console.log("render PlotlyTile.js");
